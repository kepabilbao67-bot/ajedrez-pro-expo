...
import type { AnalysisState, DifficultyLevel } from './types';
...
export async function playAiTurn({ game, engine, difficulty, signal, onStateChange }: AiTurnOptions): Promise<MoveRecord | null> {
  if (game.status().gameOver || game.getPosition().turn !== 'b') return null;
  onStateChange?.('running');
  try {
    const result = await engine.analyze({ fen: game.fen(), difficulty, limits: difficultyDefinition(difficulty).defaultLimits, signal });
